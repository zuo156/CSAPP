...
static void *address_coalesce(void *bp) {
    ...
    if (!next_alloc) {      /* coalesce with next block */
        ...
        */
        if (next == (char *)SUCC_VAL(bp)) { // using the predp and succp of bp
            // update the succ of bp
            PUT(SUCCP(bp), SUCC_VAL(next));
            // update the pred of succ of next
            PUT(PREDP(SUCC_VAL(next)), (size_t)bp);
        }
        else if (next == (char *)PRED_VAL(bp)) { // using the predp and succp of next
        ...
        else { // reusing the predp and succp of bp
...
void *mm_realloc(void *ptr, size_t size) {
    ...
    old_size = GET_SIZE(HDRP(bp))/WSIZE; // all block is already aligned

    // restore the value that will be overwritten in mm_free
    predecessor = GET(PREDP(bp));
    successor = GET(SUCCP(bp));

    mm_free(ptr);
    
    char *new_bp = mm_malloc(size);
    // copying
    PUT(new_bp, predecessor);
    PUT(new_bp + WSIZE, successor);
