...
/* 
 * rio_readlinebnew - Robustly read a text (buffered) until enter an new empty line
 */
/* $begin rio_readlineb */
ssize_t rio_readlinebnew(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    int n, rc;
    char c, *bufp = usrbuf;
    int crlf_count = 0;
        ...
        if ((rc = rio_read(rp, &c, 1)) == 1) {
            *bufp++ = c;
            ...
            if (c == '\n') {
                ...
                // do nothing, just continue to check for \n
            } else {
                crlf_count = 0;  // reset the counter if not \r or \n
            }
        } else if (rc == 0) {
            if (n == 1)
                return 0; /* EOF, no data read */
...
ssize_t Rio_readlinebnew(rio_t *rp, void *usrbuf, size_t maxlen) 
{
    ...
    if ((rc = rio_readlinebnew(rp, usrbuf, maxlen)) < 0)
	unix_error("Rio_readlinebnew error");
    return rc;
} 
